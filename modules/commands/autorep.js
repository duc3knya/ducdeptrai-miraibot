...
module.exports.run = function({ api, event, args }) {
	...
	if (content.indexOf(`del`) == 0) {
		...
		return readFile(__dirname + "/cache/shortcut.json", "utf-8", (err, data) => {
			...
			writeFile(__dirname + "/cache/shortcut.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Đã xóa autorep thành công!", threadID, messageID));
		});
	}
		...
		if (narrow == -1) return api.sendMessage("Dùng sai rồi ngu vừa thôi", threadID, messageID);
		...
		return readFile(__dirname + "/cache/shortcut.json", "utf-8", (err, data) => {
			...
			if (!oldData.some(item => item.id == threadID)) {
				...
				return writeFile(__dirname + "/cache/shortcut.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("Tạo autorep thành công", threadID, messageID));
			}
			else {
